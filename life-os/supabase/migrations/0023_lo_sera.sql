-- Life OS — Run 05, prompt 5: specchio sync del modulo Sera (stub 15).
-- SOLO FILE: scritta dalla sessione, NON applicata — il gate è di Davide.
--
-- Convenzioni identiche a 0019/0021/0022. Colonne 1:1 con
-- EveningCheckinSchema di data/schemas.ts: check-in serale (energia,
-- umore, note) + diario, che nel mondo nuovo vive in locale e sincronizza
-- come tutto il resto (su Drive ci va con l'export esplicito, stessa lib
-- di prima). "Una riga per giorno" NON è un vincolo qui: è garantito dal
-- client per costruzione (id derivato dalla data, stessa PK ovunque) — un
-- unique(user_id, date) server-side farebbe solo fallire i push in casi
-- limite senza aggiungere verità. La tabella legacy `evening_checkins`
-- resta INTATTA (sorgente read-only dell'importer).
--
-- Idempotente (rerunnabile).

create table if not exists public.lo_sera (
  id uuid not null,
  user_id uuid not null references auth.users(id) on delete cascade,
  date date not null,
  energy_1_5 smallint check (energy_1_5 is null or energy_1_5 between 1 and 5),
  mood text,
  notes text,
  journal text,
...
declare
  t text;
begin
  foreach t in array array['lo_sera']
  loop
    execute format(
...
-- ============================================================
-- RPC lo_push: stessa funzione, allowlist completa del run-05.
-- ============================================================
